...
pub async fn list_tweets(
    ...
    state
        .db
        .list_tweets(limit.unwrap_or(50), offset.unwrap_or(0), source.as_deref())
        .map_err(|e| e.to_string())
}
...
pub async fn get_thread(tweet_id: String) -> Result<ThreadData, String> {
    ...
    match clix.tweet_thread(&tweet_id) {
        Ok(tweets) => {
            let thread_tweets: Vec<ThreadTweet> = tweets
                .into_iter()
                .map(|t| {
...
#[tauri::command]
pub async fn get_tweet_notes(
    state: State<'_, AppState>,
    tweet_id: String,
) -> Result<Vec<TweetNote>, String> {
    state
        .db
        .get_tweet_notes(&tweet_id)
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn create_tweet_note(
    state: State<'_, AppState>,
    tweet_id: String,
    content: String,
) -> Result<TweetNote, String> {
    state
        .db
        .create_tweet_note(&tweet_id, &content)
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn update_tweet_note(
    state: State<'_, AppState>,
    note_id: i64,
    content: String,
) -> Result<bool, String> {
    state
        .db
        .update_tweet_note(note_id, &content)
        .map_err(|e| e.to_string())?;
    Ok(true)
}

      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-3xl mx-auto">
          ...
          <form onSubmit={handleSearch} className="flex gap-2 mb-6">
            ...
            <div className="flex rounded-xl border border-white/[0.06] overflow-hidden">
              <button type="button" onClick={() => setSearchMode("fulltext")}
                className={`px-3.5 py-2.5 text-[12px] font-medium transition-all ${searchMode === "fulltext" ? "bg-white/[0.08] text-white" : "text-white/25 hover:text-white/50"}`}>
                Exact
              </button>
              <button type="button" onClick={() => setSearchMode("semantic")}
                className={`px-3.5 py-2.5 text-[12px] font-medium transition-all ${searchMode === "semantic" ? "bg-violet-500/20 text-violet-300" : "text-white/25 hover:text-white/50"}`}>
                Semantic
              </button>
            </div>
          </form>
          ...
          <div className="space-y-2">
            {loading ? (
              <div className="text-[13px] text-white/20 py-12 text-center">Searching...</div>
            ) : tweets.length === 0 ? (
